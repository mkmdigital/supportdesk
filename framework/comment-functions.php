...
function st_comment_form_args( $args ) {
	...
	$args = array(
		...
		'must_log_in'          => '<p class="alert">' . sprintf( __( 'Sie müssen <a href="%1$s" title="Log in">eingeloggt sein</a> um einen Kommentar zu schreiben.', 'framework' ), wp_login_url( get_permalink() ) ) . '</p><!-- .alert -->',
		'logged_in_as'         => '<p class="log-in-out">' . sprintf( __( 'Eingeloggt als <a href="%1$s" title="%2$s">%2$s</a>.', 'framework' ), admin_url( 'profile.php' ), esc_attr( $user_identity ) ) . ' <a href="' . wp_logout_url( get_permalink() ) . '" title="' . esc_attr__( 'Ausloggen', 'framework' ) . '">' . __( 'Log out &raquo;', 'framework' ) . '</a></p><!-- .log-in-out -->',
		...
		'title_reply'          => __( 'Kommentar schreiben', 'framework' ),
		'title_reply_to'       => __( 'Eine Antwort auf %s schreiben', 'framework' ),
		'cancel_reply_link'    => __( 'Antwort löschen.', 'framework' ),
		'label_submit'         => __( 'Kommentar schreiben', 'framework' ),
	);
